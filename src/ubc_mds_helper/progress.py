...
def visualize_program_progress(
    ...
    capstone_progress_percentage = progress_percentage(
        program_start_date, current_date, capstone_start_date
    )

    completion_percentage = progress_percentage(
        program_start_date, current_date, program_end_date
    )
    ...
    ax.barh(
        labels, values
    )  # labels on the y axis (capsone start and program end) and completion % (values) are the bar lengths
    ax.set_xlim(0, 100)
