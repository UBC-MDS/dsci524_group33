...
**Inputs**
- `raw_grade` (float or int): original grade (0–100)
- `late_count` (int): number of previous late submissions
- `is_lower_stakes` (bool): if True, late submissions are not accepted for lower-stakes assignments (e.g., quizzes, labs)

**Outputs**
- `float`: grade after the late-submission scaling is applied
...
needed_to_pass() supports two course grading layouts, selected by course_type:
1. course_type="quiz"

lab1, lab2, lab3, lab4 are 12.5% each
quiz1, quiz2 are 25% each

2. course_type="project"

Milestone1 - Milestone4 are 20% each
IndividualAssignment1 - IndividualAssignment4 are 5% each

Inputs
1. course_type: string
Must be exactly: "quiz" or "project"

grades: dictionary
A dictionary of completed components → grade earned (0–100).
Example: {"lab1": 80, "quiz1": 70}

Dictionary rules:
- keys must be strings (component names)
- values must be numbers (int/float)
- grades must be between 0 and 100

Output
The function returns a dict of remaining components mapped to the minimum grade needed on each (same value for all remaining components).
...
{'IndividualAssignment1': 55.0, 'IndividualAssignment2': 55.0, 'Milestone2': 55.0, 'IndividualAssignment3': 55.0, 'Milestone3': 55.0, 'IndividualAssignment4': 55.0, 'Milestone4': 55.0}
